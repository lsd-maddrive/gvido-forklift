#!/usr/bin/env python3
"""
OAK-D: Depth + AprilTag детекция + Интерактивная 3D TF визуализация (matplotlib)
+ Публикация TF в ROS
"""
import matplotlib
matplotlib.use('TkAgg')  # Принудительно используем TkAgg backend
...
class DepthAprilTagDetector:
    def __init__(self):
        # Инициализация ROS
        rclpy.init()
        self.tf_node = rclpy.create_node('apriltag_tf_publisher')
        self.tf_broadcaster = TransformBroadcaster(self.tf_node)
        
        self.detector = Detector(families=TAG_FAMILY, nthreads=4)
        self.viz = MPL3DVisualizer()
        self.setup_pipeline()
        
        # Запускаем ROS спиннер в отдельном потоке
    ...
    def _spin_ros(self):
        while True:
            rclpy.spin_once(self.tf_node, timeout_sec=0.1)
    
    def publish_tf(self, tag_id, x, y, z):
        t = TransformStamped()
        t.header.stamp = self.tf_node.get_clock().now().to_msg()
    ...
    def run(self):
        ...
        with self.pipeline:
            ...
            while self.pipeline.isRunning():
                    ...
                    for tag in tags:
                        ...
                        if in_disparity is not None:
                            ...
                            if 0 <= xd < w and 0 <= yd < h:
                                ...
                                if disparity > 0:
                                    depth_m = self.depth_to_meters(disparity, CAMERA_PARAMS['fx'], CAMERA_PARAMS['baseline'] * 1000)
                        ...
                        if depth_m > 0:
                            ...
                            if pos:
                                ...
                                # Публикуем TF в ROS
                                self.publish_tf(tag.tag_id, X, Y, Z)
                ...
                if in_disparity is not None:
                    print(f"✅ Depth frame received! Shape: {in_disparity.getFrame().shape}")
                    disp_frame = in_disparity.getFrame()
